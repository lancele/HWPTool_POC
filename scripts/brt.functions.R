...
function (data,                             # the input dataframe
  ...
  plot.main = FALSE,                         # plot hold-out deviance curve
...
# setup input data and assign to position one
  ...
  if (!silent) {
    message("GBM STEP - version 2.9 \n")
    message("Performing cross-validation optimisation of a boosted regression tree model")
    message("for ",sp.name," with dataframe ",dataframe.name," and using a family of ",family, "\n")
    message("Using ",n.cases," observations and ",n.preds," predictors")
  }
...
# and then do the same for absences
    ...
    message("loading user-supplied fold vector \n")
...
# now step through the folds setting up the initial call
  ...
  if (!silent){ 
    ...
    message("\ncreating ",n.folds," initial models of ",n.trees," trees")
  
    if (prev.stratify & family == "bernoulli") message("folds are stratified by prevalence")
      else message("folds are unstratified")
  
    message ("total mean deviance = ",round(mean.total.deviance,4))

    message("tolerance is fixed at ",round(tolerance.test,4))

    if (tolerance.method != "fixed" & tolerance.method != "auto") {
      message("invalid argument for tolerance method - should be auto or fixed")
      return()}
  }
...
# now process until the change in mean deviance is =< tolerance or max.trees is exceeded
  ...
  if (verbose) message(n.fitted,"       ",round(cv.loss.values,4))
...
# calc holdout deviance
    ...
    if (j < 5) {
      if (cv.loss.values[j] > cv.loss.values[j-1]) {
        if (!silent) message("restart model with a smaller learning rate or smaller step size...")
        return()
      }
    }
      ...
      if (verbose) message(n.fitted,"      ",round(cv.loss.values[j],4))    
...
# fit the final model
  ...
  if (!silent) message("\nfitting final gbm model with a fixed number of ",target.trees," trees for ",sp.name) 
...
#extract fitted values and summary table
  ...
  if (verbose) {
    message("mean total deviance = ", round(mean.total.deviance,3))
    message("mean residual deviance = ", round(mean.resid.deviance,3))
    message("estimated cv deviance = ", round(cv.dev,3),"; se = ",
      round(cv.dev.se,3))
    message("training data correlation = ",round(self.cor,3))
    message("cv correlation = ",round(cv.cor,3),"; se = ",round(cv.cor.se,3))
    if (family == "bernoulli") {
      message("training data ROC score =",round(self.roc,3))
      message("cv ROC score = ",round(cv.roc,3),"; se =",round(cv.roc.se,3))
    }
    message("\nelapsed time - ",round(elapsed.time.minutes,2)," minutes")
  }

  if (n.fitted == max.trees & !silent) {
    message("\n","########### warning ##########","\n")
    message("maximum tree limit reached - results may not be optimal")
    message("  - refit with faster learning rate or increase maximum number of trees") 
  }
...
if (n.plots > max.vars) {
  n.plots <- max.vars
  message("warning - reducing no of plotted predictors to maximum available (",max.vars,")",sep="")
  }
...
#        where n is 25% of the number of interaction pairs;
  ...
  message("\n gbm.interactions - version 2.9")
  message("Cross tabulating interactions for gbm model with ",n.preds," predictors",sep="")
...
# extract original performance statistics...
  ...
  message("simplifying gbm.step model for ",response.name," with ",start.preds," predictors",sep="")
  message(" and ",n.cases," observations",sep="")
  message("original deviance = ",original.deviance,"(",original.deviance.se,")\n",sep="")
...
# now make predictions to the withheld fold
  ...
  while (n.steps <= n.drops & n.steps <= max.drops) {
    ...
    message("dropping predictor",n.steps)
