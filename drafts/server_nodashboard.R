...
server <- function(input, output, session) {
  ...
  # modelResults$gbm <- list(
  #   gbm = vector(mode = 'list'),
  #   valPred = vector(mode = 'list'),
  #   testPred = vector(mode = 'list'),
  #   summary = vector(mode = 'list')
  # )
  
  #### Step 1: data download option ####
  ...
  observeEvent(input$gotoStep2, {
    updateTabsetPanel(session, 'tabSteps',
                      selected = 'Step 2')
  })
  
  #### Step 3: Data pre-processing ####
  
  observeEvent(input$gotoStep3, {
    if(input$infill_model == 'skip'){
      updateTabsetPanel(session, 'tabSteps',
                        selected = 'Step 6') 
    }else{
      updateTabsetPanel(session, 'tabSteps',
  ...
  varGroup <- reactive({
    vars_meta <- dataSelect$vars_meta
    resNames <- as.list(vars_meta[vars_meta$vartype == "res", 
                                  "shortname"])
    names(resNames) <- vars_meta[vars_meta$vartype == "res", 
                                 "fullnames"]
    preNames <- as.list(vars_meta[vars_meta$vartype == "exp", 
                                  "shortname"])
    names(preNames) <- vars_meta[vars_meta$vartype == "exp", 
                                 "fullnames"]
  ...
  observe({
    ...
    updateRadioButtons(session, "varType", 
                       choices = varName())
  })
  ...
  observeEvent(input$gotoStep4, {
    ...
    updateTabsetPanel(session, 'tabSteps', selected = 'Step 4')
  })
  ...
  observeEvent(input$useModels2014, {
    withProgress(message = 'Loading 2014 models...', value = 100, {
      ...
      updateTabsetPanel(session, 'tabSteps', selected = 'Step 5')
  ...
  observeEvent(input$gotoStep6, {
    updateTabsetPanel(session, 'tabSteps', selected = 'Step 6')
  })
  ...
  observeEvent(input$explore, {
    updateTabsetPanel(session, 'tabSteps', selected = 'Explore')
  })
  ...
  #### Plot using spplot ####
  # output$shpOutDisp <- renderPlot({
  #   dataSelect$catchments$dispOut <- dataOutput$MMI[, input$dispOutVar]
  #   colr <- colorRampPalette(c('red', 'blue'))
  #   dataSelect$catchments$ColorHex <- 
  #     colr(nrow(dataSelect$catchments))[order(dataSelect$catchments$dispOut)]
  #   spplot(dataSelect$catchments, c('dispOut'), 
  #          col.regions = dataSelect$catchments$ColorHex)
  # })
  
  #### rasters ####
  # output$rasterCondition <- renderPlot({
  #   r <- raster('raster/RB1_WatershedCondition.tif')
  #   colr <- colorRampPalette(c('gray', 'steelblue'))
  #   levelplot(r,
  #             main = 'Watershed Condition',
  #             margin = F,
  #             xlab = '', ylab = '',
  #             colorkey = list(
  #               space = 'bottom',
  #               labels = list(at=0:1, font=20)
  #             ),
  #             par.settings=list(
  ...
  # })
  # 
  # output$rasterVulnerability <- renderPlot({
  #   r <- raster('raster/RB1_WatershedVulnerability.tif')
  #   colr <- colorRampPalette(c('red', 'blue'))
  #   levelplot(r,
  #             main = 'Watershed Vulnerability',
  #             margin = F,
  #             xlab = '', ylab = '',
  #             colorkey = list(
  #               space = 'bottom',
  ...
  # })
  # 
  # output$rasterHealth <- renderPlot({
  #   r <- raster('raster/RB1_StreamHealth.tif')
  #   colr <- colorRampPalette(c('brown', 'green'))
  #   levelplot(r,
  #             main = 'Stream Health',
  #             margin = F,
  #             xlab = '', ylab = '',
  ...
  output$testText <- renderPrint({
    dataOutput$MMI[c(1,10,100), input$varMMI]
  })
